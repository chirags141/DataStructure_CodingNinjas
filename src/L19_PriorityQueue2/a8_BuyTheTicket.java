...
class PriorityComparator implements Comparator<Pair>{
...
public class a8_BuyTheTicket {

    public static int buyTicketHint(int input[], int k) {
        ...
        for(int i = 0;i<input.length;i++){
            indexQueue.add(i);
            priorityHeap.add(input[i]);
        }
        System.out.println(indexQueue.peek()+" "+priorityHeap.peek());
        while(!priorityHeap.isEmpty() ){
            if(indexQueue.peek() == k && priorityHeap.peek() == input[indexQueue.peek()]){
                time++;
                return time;
            }
            else if(priorityHeap.peek() == input[indexQueue.peek()]){
                priorityHeap.remove();
                indexQueue.poll();
                time++;
            }
            else{
                int removedIndex = indexQueue.poll();
                indexQueue.add(removedIndex);
            }
        }
    ...
    public static int buyTicket(int input[], int k) {
        ...
        PriorityQueue<Pair> pq = new PriorityQueue<>(priorityComparator);
        int ansTime = 0;
        for(int i = 0; i < input.length; i++){
        ...
        while (!pq.isEmpty()){
            ...
            if((ele.index == k))
                break;;
            //            System.out.println("Priotiy: " + ele + " Index: "+ele.index);
            //            timeAns++;
        }
        return timeAns;
    }
    ...
    public static void main(String[] args) {
        int []arr =  new int[]{2,9,2,2,4};
        System.out.println(buyTicketHint(arr,2));
    }
}