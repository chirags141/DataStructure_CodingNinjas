...
public class a8_MinimumAndMaximumInABtree {

    public static Pair<Integer, Integer> getMinAndMax(BinaryTreeNode<Integer> root) {
        if(root == null){
            Pair<Integer,Integer> newPair = new Pair<Integer,Integer>(Integer.MAX_VALUE,Integer.MIN_VALUE);
            return newPair;
        }
        int max,min;
        ...
        max = Math.max(root.data,Math.max(ansLeft.maximum,ansRight.maximum));
        min = Math.min(root.data,Math.min(ansLeft.minimum,ansRight.minimum));
