...
public class a5_LongestConsecutiveSequence{
    public static ArrayList<Integer> longestConsecutiveIncreasingSequence(int[] arr) {
        ...
        // Make all those elements false which are not the beginning
        for(int i =0;i<arr.length;i++){
            int ele = arr[i];
        ...
        //Loop over again to find the list from the elements
        // which are the actual beginning of a consequence
        ArrayList<Integer> temp = new ArrayList<>();
        int maxLength = 0; int startElement = arr[0];
        for(int i =0;i<arr.length;i++){
            int ele = arr[i];
            if(map.get(ele)){
            ...
            if(temp.size() > maxLength){
                maxLength = temp.size();
                startElement = temp.get(0);
            }

        }
        ArrayList<Integer> ans = new ArrayList<>();
        ans.add(startElement);
        ans.add(startElement+maxLength-1);

        return ans;
    }

    public static void main(String[] args) {
        int[] arr = new int[]{3,7,2,1,9,8,51};
  ...
  public static ArrayList<Integer> longestConsecutiveIncreasingSequence(int[] arr) {
        ...
        for(int i = 0; i < arr.length; i++){
            ...
            while(visitedElements.containsKey(tempNum) &&
                ...
                //Mark Visited elements in the array as true
                count++;
                currentMinStartIndex =
                        elementToIndexMapping.get(tempNum);
                tempNum--;
            }
